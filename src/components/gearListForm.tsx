...
// @ts-ignore - Generated by Convex after running `npx convex dev`
...
const GearListForm = ({ tripDetails }: Props): JSX.Element => {
  ...
  const gear = useMemo(() => {
    ...
    // Check if we have local data with the same timestamp
    ...
    // Otherwise, group and mark the fresh data from Convex
  ...
  useEffect(() => {
    if (gear !== undefined) {
      localStorage.setItem(
        'gearList',
        JSON.stringify({ timestamp: tripDetails.timestamp, data: gear })
      );
    }
  }, [gear]);
  ...
  const updateGearList = (newList: GroupedGearListToPack): void => {
    localStorage.setItem(
      'gearList',
      JSON.stringify({ timestamp: tripDetails.timestamp, data: newList })
    );
  ...
  const addItem = (selectedItem: SingleValue<SelectOption>): void => {
    ...
    const updatedGear = {
      ...gear,
      [value.group]: [...gear[value.group], value.item],
    };
  ...
  const isItemAlreadyOnList = (itemName: string): boolean => {
    ...
    return Object.values(gear).some((group) =>
